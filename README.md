...
Problem statement: You need to build a scratch game, that will generate a matrix (for example 3x3) from symbols(based on probabilities for each individual
 cell) and based on winning combintations user either will win or lost.
...
repeated symbols or symbols that follow a specific pattern) Bonus symbols are described the table below:
...
the generated matrix. Bonus symbols are described the table below:
...
- **Note:** Fields which are marked as OPTIONAL, are not required but will add extra points to the candidate if the candidate 
implements it.

- **Note (2):** Bonus symbol can be generated randomly in any cell(s) in the matrix

- **Note (3):** If one symbols matches more than winning combinations then reward should be multiplied. formula: 
...
| field name                   | description                                            |
|------------------------------|--------------------------------------------------------|
| matrix                       | generated 2D matrix                                    |ok
| reward                       | final reward which user won                            |ok
| applied_winning_combinations | Map of Symbol and List of applied winning combinations |ok
| applied_bonus_symbol         | applied bonus symbol (can be null if the bonus is MISS |ok
...
</br> "applied_winning_combinations": {
</br> "B": ["same_symbol_3_times"] </br> }, 
</br> "applied_bonus_symbol": "10x" </br> } 
|
